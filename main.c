...
void Send_measurement_message(SOCKET socket1, const char *sendbufp, int bytesp, unsigned int ts_current,
                              unsigned short *seq_nump, struct sockaddr_in *serverp);

static int info2=0;
static int info3=0;
RTP_FIXED_HEADER *rtp_hdr, *rtp_hdrp;
...
int main(int argc, char* argv[])
{
    ...
    SOCKET socket1;
    char  *nalu_payload;
    char  sendbuf[1500],sendbufp[Measure_message_length];
    int   len ,lenp;
    int   bytes = 0,bytesp = 0;
    float framerate = 25;
    unsigned short seq_num = 0,seq_nump = 0;
    unsigned int timestamp_increase = 0;
    unsigned int ts_current = 0;
    struct sockaddr_in server,serverp;

    len = sizeof(server);
    lenp = sizeof(serverp);
    OpenBitstreamFile(File_location);
    ...
    serverp.sin_port = htons(DEST_PORT1);
    serverp.sin_addr.s_addr = inet_addr(DEST_IP);
    ...
    while(!feof(bits))
    {
        ...
        memset(sendbufp, 0, Measure_message_length);//清空sendbuf；此时会将上次的时间戳清空，因此需要ts_current来保存上次的时间戳值
        //rtp固定包头，为12字节,该句将sendbuf[0]的地址赋给rtp_hdr，以后对rtp_hdr的写入操作将直接写入sendbuf。
        rtp_hdrp =(RTP_FIXED_HEADER*)&sendbufp[0];
        //设置RTP HEADER，
        rtp_hdrp->csrc_len	 = 0;
        //rtp_hdr->extension=0;
        //rtp_hdr->padding=0;
        rtp_hdrp->payload     = PRTP;  		//负载类型号，
        rtp_hdrp->version     = 2;  			//版本号，此版本固定为2
        rtp_hdrp->marker      = 0;   		//标志位，由具体协议规定其值。
        rtp_hdrp->ssrc        = htonl(20); 	//随机指定为10，并且在本RTP会话中全局唯一  bytes 8-11
        ...
        if(n->len<=1400)
        {
            ...
            Send_measurement_message(socket1, sendbufp, bytesp, ts_current, &seq_nump, &serverp);
...
void Send_measurement_message(SOCKET socket1, const char *sendbufp, int bytesp, unsigned int ts_current,
    ...
    struct timeval tv;
    rtp_hdrp->marker = 1;
    rtp_hdrp->seq_no = htons((*seq_nump) ++);
